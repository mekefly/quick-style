...
//渲染
const sayDom = document.querySelector(".say");
const nameDom = document.querySelector(".name");
const locationDom = document.querySelector(".location");
const portraitDom = document.querySelector(".portrait");
let index = 0;
function render() {
  const data = list[index];
  sayDom.innerText = data.say;
  nameDom.innerText = data.name;
  locationDom.innerText = data.location;
  portraitDom.style.backgroundImage = `url(${data.portrait})`;
}
render();
...
progressDom.addEventListener("animationiteration", update);
function update() {
  index++;
  if (index >= len) {
    index = 0;
